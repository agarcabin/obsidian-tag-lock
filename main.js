...
var PrivacyGuardSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  getSettingDefinitions() {
    ...
    const toggle = (name, desc, key, defaultValue) => ({
      name,
      desc,
      control: { type: "toggle", key, defaultValue }
    });
    const dropdown = (name, desc, key, options, defaultValue) => ({
      name,
      desc,
      control: { type: "dropdown", key, options, defaultValue }
    });
    const textArea = (name, desc, key, defaultValue) => ({
      name,
      desc,
      control: { type: "textarea", key, rows: 3, defaultValue }
    });
    const text = (name, desc, key, defaultValue) => ({
      name,
      desc,
      control: { type: "text", key, defaultValue }
    });
    return [
      {
        type: "group",
        ...
        items: [
          {
            ...
            control: { type: "number", key: "unlockMinutes", min: 0, defaultValue: DEFAULT_SETTINGS.unlockMinutes }
          },
  ...
  getControlValue(key) {
    switch (key) {
      case "protectedTags":
        return this.plugin.settings.protectedTags.join("\n");
      case "protectedFolders":
        return this.plugin.settings.protectedFolders.join("\n");
      case "unlockMinutes":
        return this.plugin.settings.unlockMinutes;
      case "protectionEnabled":
        return this.plugin.settings.protectionEnabled;
      case "language":
        return this.plugin.settings.language;
      case "protectTagNamedLinks":
        return this.plugin.settings.protectTagNamedLinks;
      case "protectTagNamedPages":
        return this.plugin.settings.protectTagNamedPages;
      case "protectSearch":
        return this.plugin.settings.protectSearch;
      case "protectPreview":
        return this.plugin.settings.protectPreview;
      case "protectSettings":
        return this.plugin.settings.protectSettings;
      case "authMethod":
        return this.plugin.settings.authMethod;
      case "passwordHint":
        return this.plugin.settings.passwordHint;
      case "unlockOnCorrectCredential":
        return this.plugin.settings.unlockOnCorrectCredential;
      case "failureAction":
        return this.plugin.settings.failureAction;
      case "lockOnRestart":
        return this.plugin.settings.lockOnRestart;
      case "lockOnBackground":
        return this.plugin.settings.lockOnBackground;
      case "forcePasswordEveryAccess":
        return this.plugin.settings.forcePasswordEveryAccess;
      default:
        return void 0;
    }
  }
